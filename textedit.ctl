...
Public Function EditWindowProc(ByVal hwnd As Long, ByVal uMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
    ...
    Case WM_COMMAND:
        ...
        Case EN_CHANGE:
            RaiseEvent Changed
        End Select
'    Case WM_PAINT:  'Без этого может прекратиться перерисовка родительского окна
...
With UserControl
    ...
    End If
    If editwnd = 0 Then 'Вообще, если вызывается init, это условие нужно выполнить
...
Public Property Let Multiline(ByVal bool As Boolean)
    If bool = multilined Then Exit Property 'Незачем пересоздавать окно и менять свойство
If RuntimeMode Then
    Dim txt As String
    txt = text
...
'Эмулирует рамку заданной толщины и цвета
